    ...
    for i, (x_data, control_data, label) in enumerate(train_loader):
        ...
        if _lam_g_trj > 0:
            ...
            for tp in range(trjloss_sampling_points):
                ...
                g_trj_loss += _lam_g_trj * 0.5 * base_criterion(fake_trajectory[:,:(tp+1)*sampling_interval-1,0], control[:,:,(tp+1)*sampling_interval-1,0])
                g_trj_loss += _lam_g_trj * 0.5 * base_criterion(fake_trajectory[:,:(tp+1)*sampling_interval-1,1], control[:,:,(tp+1)*sampling_interval-1,1])
                g_trj_loss += _lam_g_trj * 0.5 * base_criterion(fake_trajectory[:,:(tp+1)*sampling_interval-1,2], control[:,:,(tp+1)*sampling_interval-1,2])
